...
| Name | Credit | Comment | Screenshots | Reasons not to install |
|------|--------|---------|-------------|------------------------|
...
| enableExtraPenOnMove.qmd | [ingatellent](https://github.com/ingatellent/xovi-qmd-extensions) | It's a shame that reMarkable decided to remove such a useful feature on [Paper Pro Move](https://remarkable.com/products/remarkable-paper/pro-move). | ![](screenshots/enableExtraPenOnMove.jpg) | It eats up space in the toolbar. |
| eraseSelection.qmd | [FouzR](https://github.com/FouzR/xovi-extensions)¹ | Allows using selection tool to erase. Compared to the FouzR's version, erasing preserves current clipboard contents. | ![](screenshots/eraseSelection.jpg) | N/A |
| forceRefreshGesture.qmd | [asivery](https://github.com/asivery/rm-hacks-qmd) | Force refresh the screen by touching it with five fingers. It's a built-in (though disabled by default) feature of xochitl. It helps remove any accumulated ghosting. | N/A | N/A |
| frontLightIcon.qmd | myself | Displays a small icon in the upper right-hand corner whenever front light is enabled and toolbar is hidden. | ![](screenshots/frontLightIcon.jpg) | Might ruin the minimalitic design if you use front light often. |
| gestures.qmd | myself | Simple gestures to switch between pens and the selection tool. | ![](screenshots/gestures.jpg) | It creates extra learning curve for the users, and makes scrolling more difficult (you need to swipe strictly in the center). |
| hideCloseButton.qmd | [asivery](https://github.com/asivery/rm-hacks-qmd) | Removes the button that I never use. Doing so frees up some space in the toolbar (e.g. for the second pen). | ![](screenshots/hideCloseButton.jpg) | It creates confusion for the first-time reMarkable users (until they learn that a swipe from the top closes the document). |
...
| toolbarIcon.qmd | [FouzR](https://github.com/FouzR/xovi-extensions)¹ | Displays some info about the currently selected tool inside the hide/show toolbar button, but in a very non-invasive way. When black pen is selected, it is designed to not show any changes. | ![](screenshots/toolbarIcon.jpg) | It is not as useful without gestures. |
| uncompressDock.qmd | [NohamR](https://github.com/NohamR/xovi-qmd-extensions) | Replace the + button in the document navigator with the actual icons. | ![](screenshots/uncompressDock.jpg) | It is not immediately clear what the buttons do, unless you have used them before. |
